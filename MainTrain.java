...
public class MainTrain {
    ...
    static void checkCorrelationTrain(CorrelatedFeatures c,String f1, String f2, float a, float b){
        if(c.feature1.equals(f1)){
            if(!c.feature2.equals(f2))
                System.out.println("wrong correlated feature of "+f1+"");
            else{
                if(c.corrlation<0.99)
                    System.out.println(f1+"-"+f2+" wrong correlation detected");
                if(c.lin_reg.a<a-0.5f || c.lin_reg.a>a+0.5f)
                    System.out.println(f1+"-"+f2+" wrong value of line_reg.a");
                if(c.lin_reg.b<b-0.5f || c.lin_reg.b>b+0.5f)
                    System.out.println(f1+"-"+f2+" wrong value of line_reg.b");
                if(c.threshold>0.3)
                    System.out.println(f1+"-"+f2+" wrong threshold detected");
            }
        }
    ...
    public static void main(String[] args) {
        ...
        if(cf.size()!=2)
            System.out.println("wrong size of correlated features");
        else
        ...
        if(!anomlyDetected)
            System.out.println("the anomaly was not detected");
